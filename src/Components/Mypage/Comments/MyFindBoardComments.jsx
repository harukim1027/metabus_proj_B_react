...
function MyFindBoardComments() {
  ...
  const [{ data: commentList, loading, error }, refetch] = useApiAxios(
    {
      url: `/find_owner_board/api/comments/`,
      method: 'GET',
    },
    {
      manual: true,
                ...
                {commentList && (
                  <>
                    {commentList.results
                      ...
                      .map((lostpetcomments) => (
                        <tr
                          key={lostpetcomments.find_comment_no}
                          onClick={() =>
                            navigate(
                              `/findboard/${lostpetcomments.find_board_no.find_board_no}/`,
                            )
                          }
                          className="cursor-pointer"
                        >
                          <td className="py-4 lg:text-xl md:text-base sm:text-sm xs:text-xxs">
                            {lostpetcomments.find_comment_no}
                          </td>
...
export default MyFindBoardComments;
